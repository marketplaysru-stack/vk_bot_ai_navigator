...
def generate_template_text(topic: str) -> str:
    ...
    Использует множество заготовок, чтобы посты не повторялись.
    """
    # --- Хуки (5-7 слов) ---
    hooks = [
    ...
    # --- Тело – пункты (3-5) ---
    body_pool = [
        ("🔥", "Автоматизация рутины", "ИИ берёт на себя скучные задачи: от обработки документов до планирования. Ты освобождаешь время для творчества. Например, нейросети уже пишут тексты и составляют отчёты."),
        ("💡", "Анализ данных", "Нейросети находят закономерности, которые человек не видит. Это помогает принимать решения быстрее и точнее. В бизнесе это сокращает риски."),
        ("🧘", "Этика и безопасность", "Важно помнить: ИИ – это не замена человека, а помощник. Вопросы приватности и ответственности уже решаются на уровне законодательства."),
        ("⚡", "Скорость внедрения", "Технологии развиваются экспоненциально. То, что казалось фантастикой вчера, сегодня – реальность. Уже есть ИИ-ассистенты для юристов и врачей."),
        ("📌", "Обучение и адаптация", "Чтобы быть в тренде, нужно постоянно учиться. Нейросети уже помогают в образовании и переквалификации. Онлайн-курсы с ИИ-тьюторами – реальность."),
        ("🚀", "Инновации и стартапы", "ИИ открывает новые рынки. Стартапы на базе AI растут быстрее и привлекают инвестиции. Это шанс для предпринимателей."),
        ("💪", "Конкурентоспособность", "Компании, которые внедряют AI, увеличивают прибыль на 30–40% и сокращают издержки."),
        ("🔍", "Прогнозирование", "ИИ помогает предсказывать тренды, спрос, риски – это даёт преимущество в любой сфере."),
        ("🛡️", "Кибербезопасность", "Нейросети защищают от атак быстрее человека, анализируя аномалии в реальном времени."),
        ("🌍", "Глобальные решения", "ИИ помогает решать проблемы экологии, медицины, логистики – меняет мир к лучшему. Например, прогнозирование погоды и климата.")
    ]
    ...
    conclusions = [
        ...
        "Главное – не бояться, а учиться. ИИ – это инструмент, который делает нас сильнее и свободнее.",
        "Технологии приходят, чтобы помочь. От нас зависит, как мы их используем. Будьте открыты новому."
    ]
...
def generate_agnes_image(prompt):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
        full_prompt = f"Professional illustration about {prompt}, AI, technology, neural networks, creative, modern, high quality, no people, no nature"
        data = {
            "prompt": full_prompt,
...
def generate_pollinations_image(prompt):
    try:
        seed = random.randint(1, 1000000)
        full_prompt = f"{prompt}, AI, technology, creative, modern, high quality, no people, no nature"
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст, чтобы он стал более живым, добавь эмодзи, абзацы, сделай его как пост популярного блогера об ИИ. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 600,
