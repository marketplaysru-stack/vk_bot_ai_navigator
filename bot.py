...
# -*- coding: utf-8 -*-
"""
Бот с автопостингом, RSS и генерацией текста/картинок
Поддерживает:
- /post <тема> — генерирует текст и картинку
- /post <длинный текст> — использует текст как пост, генерирует картинку
"""
...
def generate_text(topic):
    """Генерирует текст поста (∼200 слов) через Agnes или шаблон."""
    if AGNES_API_KEY:
        try:
            headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
            data = {
                "model": "agnes-v1",
                "messages": [{"role": "user", "content": f"Напиши развёрнутый пост (около 200 слов) на тему: {topic}. Используй факты, примеры, выводы. Пиши в деловом, но доступном стиле."}],
                "max_tokens": 400,
                "temperature": 0.7
            }
            resp = requests.post("https://api.agnes.ai/v1/chat/completions", json=data, headers=headers, timeout=30)
            if resp.status_code == 200:
                result = resp.json()
...
def generate_template_text(topic):
    intro_phrases = [
...
def create_post_from_topic(topic):
    """Создаёт пост из короткой темы (генерирует и текст, и картинку)."""
    ...
    return text, announce_text, group_link, image_bytes

def create_post_from_text(user_text):
    """Использует готовый текст пользователя, генерирует картинку по теме."""
    # Извлекаем тему для картинки (первые 50 символов)
    topic = user_text[:50] + ("..." if len(user_text) > 50 else "")
    image_bytes = generate_image(topic)
    announce_text = f"🔥 Новый пост в группе AI Навигатор"
    group_link = f"https://vk.com/club{abs(GROUP_ID_AI)}" if GROUP_ID_AI < 0 else f"https://vk.com/public{GROUP_ID_AI}"
    return user_text, announce_text, group_link, image_bytes
...
def handle_command(chat_id, text):
    if text in ("/start", "/help"):
        send_message(chat_id,
            "👋 Бот для постов в ВК\n\n"
            "📌 Команды:\n"
            "/post <текст> — опубликовать пост (если текст длинный — используем как есть, если короткий — генерируем)\n"
            "/post <тема> — сгенерировать пост по теме\n"
    ...
    if text.startswith("/post"):
        ...
        if not content:
            send_message(chat_id, "❌ Укажите тему или текст. Пример: /post Нейросети в бизнесе")
            return

        send_message(chat_id, "⏳ Обрабатываю запрос...")

        # Если текст длиннее 100 символов — считаем его готовым постом
        if len(content) > 100:
            post_text, announce_text, group_link, image_bytes = create_post_from_text(content)
            logger.info("Использован готовый текст пользователя")
        else:
            post_text, announce_text, group_link, image_bytes = create_post_from_topic(content)
            logger.info("Сгенерирован текст по теме")

        # Публикуем в группу
        group_result = publish_to_group(post_text, image_bytes)
        send_message(chat_id, f"📌 Группа:\n{group_result}")

        # Публикуем анонс на личную стену
