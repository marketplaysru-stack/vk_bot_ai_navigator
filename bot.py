...
# -*- coding: utf-8 -*-
"""
Простой бот с генерацией картинок через Agnes, Pexels, баннер (исправлена загрузка фото)
"""
...
# ========== ЗАГРУЗКА ФОТО В VK (ИСПРАВЛЕННАЯ) ==========
def upload_photo_to_vk(image_bytes):
    ...
    logger.info(f"Загрузка фото в VK для группы {group_id_abs}")
    ...
    logger.info(f"Upload URL получен: {upload_url[:50]}...")
    ...
    logger.info(f"Ответ сохранения: {json.dumps(save_resp, indent=2)}")
