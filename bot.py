...
Картинки генерируются через нейросети с футуристическими, AI-промптами.
Фиксированный порядок генераторов: Agnes → Pixazo → Pollinations → Pexels → баннер.
...
def get_updates(offset=None):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(f"Неожиданный статус {resp.status_code}: {resp.text}")
            return []
...
# ---------- УЛУЧШЕННЫЕ ГЕНЕРАТОРЫ КАРТИНОК (футуристика, AI, роботы) ----------
...
def generate_agnes_image(prompt):
    logger.info("🔄 Пытаемся получить картинку через Agnes...")
...
def generate_pixazo_image(prompt):
    logger.info("🔄 Пытаемся получить картинку через Pixazo...")
    if not PIXAZO_API_KEY:
        logger.warning("Pixazo: ключ не задан")
        return None
    try:
        seed = random.randint(1, 1000000)
        full_prompt = (
        ...
        if resp.status_code == 200:
            ...
            if image_url:
                img = download_image_with_retry(image_url)
                if img:
                    logger.info("Pixazo: картинка получена")
                    return img
                else:
                    logger.warning("Pixazo: не удалось скачать изображение")
            else:
                logger.warning("Pixazo: ответ не содержит URL")
        else:
            logger.warning(f"Pixazo: статус {resp.status_code}")
    except Exception as e:
        logger.warning(f"Pixazo: исключение {e}")
    return None
...
def generate_image(topic):
    clean_used_images()
    # Фиксированный порядок: Agnes -> Pixazo -> Pollinations -> Pexels -> баннер
    generators = [
...
# ---------- ЗАГРУЗКА ФОТО (ПРЯМОЙ HTTP) С УЛУЧШЕННОЙ ОБРАБОТКОЙ ----------
...
# ---------- ОБРАБОТЧИК КОМАНД С ЛОГИРОВАНИЕМ ----------
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 AI Навигатор запущен (футуристические картинки, фиксированный порядок генераторов)")
