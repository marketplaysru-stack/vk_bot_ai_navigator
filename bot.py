...
Порядок: Agnes → Replicate → Pollinations → Pexels → баннер.
Таймауты увеличены до 180 секунд для качественной генерации.
...
REPLICATE_API_KEY = os.getenv("REPLICATE_API_KEY")   # новый ключ
...
# ---------- ГЕНЕРАТОРЫ КАРТИНОК (с Replicate) ----------
        ...
        if resp.status_code == 200:
            ...
            if image_url:
                ...
                if img:
                    ...
                    logger.warning("Agnes: не удалось скачать изображение")
            else:
...
def generate_replicate_image(prompt):
    logger.info("🔄 Пытаемся получить картинку через Replicate (таймаут 180 сек)...")
    if not REPLICATE_API_KEY:
        logger.warning("Replicate: ключ не задан")
        return None
    try:
        # Используем модель SDXL для высокого качества
        ...
        payload = {
            ...
            "input": {
                "prompt": full_prompt,
                "negative_prompt": "ugly, deformed, blurry, low quality, people, human, woman, girl, nature",
                "width": 1280,
                "height": 1280,
                "num_inference_steps": 45,
                "guidance_scale": 7.5,
        ...
        # Запускаем генерацию
        create_resp = requests.post("https://api.replicate.com/v1/predictions", json=payload, headers=headers, timeout=180)
        if create_resp.status_code != 201:
            logger.warning(f"Replicate: ошибка создания задачи {create_resp.status_code}")
            return None
        prediction = create_resp.json()
        # Ждём завершения
        get_url = prediction["urls"]["get"]
        for _ in range(60):  # максимум 60 попыток по 3 секунды = 180 секунд
            status_resp = requests.get(get_url, headers=headers, timeout=30)
            if status_resp.status_code == 200:
                status_data = status_resp.json()
                if status_data["status"] == "succeeded":
                    image_url = status_data["output"][0]  # SDXL возвращает список ссылок
                    if image_url:
                        img = download_image_with_retry(image_url)
                        if img:
                            logger.info("Replicate: картинка получена")
                            return img
                        else:
                            logger.warning("Replicate: не удалось скачать изображение")
                    else:
                        logger.warning("Replicate: ответ не содержит URL")
                    break
                elif status_data["status"] in ["failed", "canceled"]:
                    logger.warning(f"Replicate: генерация завершилась с ошибкой {status_data.get('error')}")
                    break
                else:
                    time.sleep(3)
            else:
                logger.warning(f"Replicate: ошибка проверки статуса {status_resp.status_code}")
                time.sleep(3)
        else:
            logger.warning("Replicate: таймаут ожидания генерации")
    except Exception as e:
        logger.warning(f"Replicate: исключение {e}")
    return None
...
def generate_image(topic):
    clean_used_images()
    # Фиксированный порядок: Agnes -> Replicate -> Pollinations -> Pexels -> баннер
    generators = [
        ("Agnes", generate_agnes_image),
        ("Replicate", generate_replicate_image),
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 AI Навигатор запущен (с Replicate, таймауты 180 сек)")
