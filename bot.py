...
Ключи читаются ТОЛЬКО из переменных окружения. Ни .env, ни secrets.txt.
"""
...
def _diag():
    """Диагностика: какие переменные реально видны процессу."""
    ...
    logger.error("   Доступные переменные, содержащие VK/TOKEN/HF/PEXELS:")
    found = False
    for k, v in os.environ.items():
        ku = k.upper()
        if any(s in ku for s in ("VK", "TOKEN", "HF", "HUGGINGFACE", "PEXELS")):
