...
BOT_TOKEN = os.getenv("BOT_TOKEN")              # Токен AI Telegram-бота
...
log("🚀 Запуск AI-бота (Agnes → GigaChat → Pollinations, с отдельными настройками, VK Feeds)")
...
def execute_scheduled_post(item):
    ...
    niche = "ai"
    topic = item["topic"]
    log(f"📢 Публикую запланированный пост: '{topic}' (AI)")
...
def process_message(message):
    ...
    if text.startswith("/run_now"):
        ...
        def publish():
            item = {"niche": "ai", "topic": topic, "time": datetime.now().strftime("%Y-%m-%d %H:%M")}
            execute_scheduled_post(item)
...
def add_test_post_if_empty():
    ...
    if not has_ai:
        ...
        schedule.append({
            ...
            "niche": "ai",
            "topic": "Тестовый пост для AI-бота (гиперреалистичные картинки)",
            "time": test_time,
            "done": False
        })
        save_schedule(schedule)
...
if __name__ == "__main__":
    log("🤖 AI-бот (с отдельными настройками и VK Feeds) запущен")
