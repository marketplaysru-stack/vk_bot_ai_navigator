...
Исправлена загрузка фото через прямой HTTP.
"""
...
def upload_photo_to_vk_via_http(image_bytes, owner_id, token):
    """
    Загружает фото на стену через прямой HTTP-запрос.
    Для owner_id < 0 (группа) – передаём group_id.
    Для owner_id > 0 (публичная страница/личная стена) – без group_id.
    ...
    try:
        ...
        if owner_id < 0:
            ...
            logger.info(f"Загрузка на личную стену или публичную страницу (owner_id={owner_id})")
