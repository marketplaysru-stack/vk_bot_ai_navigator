...
def generate_agnes_image(prompt):
    ...
    try:
        ...
        # Футуристический промпт для AI-тематики
        full_prompt = (
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст, чтобы он стал более żywy, добавь emoji, akapity, сделай jak пост popularnego blogera o ИИ. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 600,
