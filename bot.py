...
# -*- coding: utf-8 -*-
"""
AI Навигатор – генерация постов с динамическими картинками.
Промпты зависят от темы поста, используются яркие цвета и разные стили.
"""
...
AGNES_API_KEY = os.getenv("AGNES_API_KEY")  # для текста
...
# ---------- ГЕНЕРАТОРЫ КАРТИНОК (динамические промпты) ----------
...
def build_prompt(topic: str) -> str:
    """
    Строит уникальный промпт на основе темы поста.
    Добавляет случайные цвета, стили и атрибуты.
    """
    # Определяем категорию темы
    topic_lower = topic.lower()
    if any(w in topic_lower for w in ['бизнес', 'стартап', 'компания', 'предприятие']):
        category = "business"
    elif any(w in topic_lower for w in ['медицин', 'здоровье', 'диагност', 'лечение']):
        category = "medical"
    elif any(w in topic_lower for w in ['творчество', 'искусство', 'дизайн', 'музык']):
        category = "creative"
    elif any(w in topic_lower for w in ['робот', 'механик', 'автоматиз']):
        category = "robotics"
    else:
        category = "general"

    # Цветовые палитры (яркие, не тёмные)
    color_palettes = {
        "business": ["gold", "blue", "cyan", "white", "silver"],
        "medical": ["teal", "green", "white", "light blue", "purple"],
        "creative": ["pink", "orange", "violet", "yellow", "magenta"],
        "robotics": ["red", "orange", "yellow", "blue", "white"],
        "general": ["cyan", "magenta", "yellow", "white", "gold", "neon green"]
    }
    palette = color_palettes.get(category, color_palettes["general"])
    primary_color = random.choice(palette)
    secondary_color = random.choice(palette)
    # Чтобы избежать одинаковых цветов
    while secondary_color == primary_color and len(palette) > 1:
        secondary_color = random.choice(palette)

    # Стили
    style = random.choice(["cyberpunk", "synthwave", "futuristic", "3D render", "photorealistic", "abstract", "digital art", "neon glow", "vaporwave"])

    # Атрибуты робота/ИИ
    if category == "business":
        attributes = ["sleek", "modern", "corporate", "minimalist", "metallic"]
        objects = ["holographic charts", "data graphs", "financial symbols", "office environment"]
    elif category == "medical":
        attributes = ["biotech", "organic", "clean", "high-tech", "precision"]
        objects = ["DNA helix", "medical cross", "scanner", "laboratory"]
    elif category == "creative":
        attributes = ["vibrant", "artistic", "colorful", "expressive", "unique"]
        objects = ["paint splashes", "musical notes", "geometric patterns", "abstract shapes"]
    elif category == "robotics":
        attributes = ["robust", "mechanical", "articulated", "powerful", "dynamic"]
        objects = ["gears", "circuit boards", "mechanical arm", "power cells"]
    else:
        attributes = ["futuristic", "high-tech", "intelligent", "advanced"]
        objects = ["data streams", "digital brain", "holographic displays", "neural networks"]
    ...
    prompt = (
        f"futuristic robot, {attr}, {style}, {primary_color} and {secondary_color} color scheme, "
        f"artificial intelligence, glowing circuits, {obj}, "
        f"highly detailed, sharp, no people, no nature"
    )
    return prompt

# ----- Hugging Face (с несколькими моделями) -----
def generate_huggingface_image(topic):
    if not HUGGINGFACE_API_KEY:
        logger.warning("Hugging Face: ключ не задан")
        return None
    logger.info("🔄 Пытаемся получить картинку через Hugging Face...")
    prompt = build_prompt(topic)
    logger.info(f"HF промпт: {prompt[:100]}...")
    models = [
...
# ----- Pollinations (улучшенный) -----
def generate_pollinations_image(topic):
    logger.info("🔄 Пытаемся получить картинку через Pollinations...")
    prompt = build_prompt(topic)
    logger.info(f"Pollinations промпт: {prompt[:100]}...")
    try:
        seed = random.randint(1, 1000000)
        url = f"{POLLINATIONS_BASE_URL}/prompt/{requests.utils.quote(prompt)}?width=1920&height=1920&nologo=true&seed={seed}&model=flux&upscale=true"
...
def generate_image(topic):
    ...
    # Баннер (тоже меняем цвет фона случайно)
...
def create_banner(text, width=1024, height=1024):
    # Случайный яркий фон
    bg_colors = ["#0a0a2e", "#1a1a4e", "#2a1a5e", "#0a2a2e", "#1a3a2e", "#2a2a1a"]
    ...
    text_colors = ["#FFD700", "#00FFFF", "#FF00FF", "#00FF00", "#FF4500", "#FFFFFF"]
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 AI Навигатор запущен (динамические промпты, яркие цвета)")
