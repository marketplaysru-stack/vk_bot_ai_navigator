...
Уведомления в Телеграм: chat_id определяется автоматически.
"""
...
# Ключи — только из переменных окружения
...
TELEGRAM_CHAT_ID = _env("TELEGRAM_CHAT_ID", "AI_CHAT_ID")  # можно не задавать — определится автоматически
...
def tg_init():
    """Определяет chat_id и очищает вебхук. Возвращает chat_id или None."""
    global TELEGRAM_CHAT_ID
    if not TELEGRAM_TOKEN:
        logger.info("📨 Telegram: токен не задан, уведомления отключены")
        return None

    # 1) Проверяем, что токен живой
    me = tg_api("getMe")
    if not me or not me.get("ok"):
        logger.warning(f"📨 Telegram: токен невалиден ({me})")
        return None
    bot_username = me["result"].get("username", "?")
    logger.info(f"📨 Telegram: бот @{bot_username}")

    # 2) Удаляем вебхук (иначе getUpdates не отдаст сообщения)
    tg_api("deleteWebhook", drop_pending_updates=False)

    # 3) Если chat_id уже задан в env — используем его
    if TELEGRAM_CHAT_ID:
        logger.info(f"📨 Telegram chat_id из переменных: {TELEGRAM_CHAT_ID}")
        return TELEGRAM_CHAT_ID

    # 4) Иначе — ищем в getUpdates
    upd = tg_api("getUpdates", timeout=1)
    if upd and upd.get("ok") and upd.get("result"):
        # последний апдейт от пользователя
    ...
    # 5) Не нашли — просим написать боту /start
    logger.warning(f"📨 Telegram: chat_id не найден. Напиши /start боту @{bot_username} и перезапусти бота")
    return None

def tg_send(text):
    """Отправляет уведомление в Телеграм, если есть токен и chat_id."""
...
def post_now():
    ...
    if ok:
        tg_send(
            f"🚀 <b>[{BOT_NAME}]</b> Пост опубликован\n\n"
            f"<b>Тема:</b> {topic}\n"
            f"<b>Группа:</b> {GROUP_ID}\n\n"
            f"{text[:800]}"
        )
    else:
        tg_send(f"❌ <b>[{BOT_NAME}]</b> Не удалось опубликовать пост в группу {GROUP_ID}")
...
def main():
    ...
    logger.info(f"🔑 VK: {'есть' if VK_TOKEN else 'НЕТ'}" + (f" ({VK_TOKEN[:12]}...)" if VK_TOKEN else ""))
    logger.info(f"🔑 HF: {'есть' if HUGGINGFACE_KEY else 'НЕТ'}")
    logger.info(f"🔑 Pexels: {'есть' if PEXELS_KEY else 'НЕТ'}")

    # Инициализация Телеграма (определит chat_id)
    tg_init()
    if TELEGRAM_CHAT_ID:
        tg_send(f"🟢 <b>[{BOT_NAME}]</b> запущен\nГруппа: {GROUP_ID}\nРасписание: {', '.join(POST_TIMES)}")
...
def _diag():
    logger.error("❌ Не найден VK-токен в переменных окружения")
    logger.error("   Пробованные имена: VK_TOKEN_AI, AI_VK_TOKEN, VK_TOKEN")
    logger.error(f"   Рабочая папка: {os.getcwd()}")
    logger.error("   Доступные переменные, содержащие VK/TOKEN/HF/PEXELS/TELEGRAM:")
    found = False
    for k, v in os.environ.items():
        ku = k.upper()
        if any(s in ku for s in ("VK", "TOKEN", "HF", "HUGGINGFACE", "PEXELS", "TELEGRAM")):
            status = "ЗАДАНА" if v.strip() else "ПУСТА"
            logger.error(f"     {k}: {status}")
            found = True
    if not found:
        logger.error("     (ни одной — переменные не передаются в процесс)")
...
if __name__ == "__main__":
    ...
    # Тестовый режим: python bot.py test — один пост и выход
    if len(sys.argv) > 1 and sys.argv[1].lower() == "test":
        logger.info("🧪 ТЕСТОВЫЙ РЕЖИМ — один пост и выход")
        tg_init()
        if TELEGRAM_CHAT_ID:
            tg_send(f"🧪 <b>[{BOT_NAME}]</b> тестовый режим: сейчас сделаю один пост")
        post_now()
        logger.info("🏁 Тестовый пост завершён")
        sys.exit(0)
