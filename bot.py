...
Порядок: Hugging Face → Pollinations → Pexels → баннер.
Все ключи берутся только из переменных окружения.
"""
...
# Все ключи – только из окружения, без значений по умолчанию!
...
def generate_text(topic: str) -> str:
    # Убираем Agnes из генерации текста, используем только шаблон (или можно оставить Agnes, но без ключа)
...
# ---------- ГЕНЕРАТОРЫ КАРТИНОК (без жёстких ключей) ----------
...
def generate_huggingface_image(prompt):
    if not HUGGINGFACE_API_KEY:
        logger.warning("Hugging Face: ключ не задан в переменных окружения")
        return None
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if is_valid_image(img):
                logger.info("Hugging Face: картинка получена")
                return img
            else:
                logger.warning("Hugging Face: изображение повреждено или невалидно")
        elif resp.status_code == 503:
            logger.warning("Hugging Face: модель загружается, ждём...")
            time.sleep(5)
            return generate_huggingface_image(prompt)
        else:
            logger.warning(f"Hugging Face: статус {resp.status_code} - {resp.text[:200]}")
    except Exception as e:
...
def search_pexels_relevant_photo(topic):
    if not PEXELS_API_KEY:
        logger.warning("Pexels: ключ не задан в переменных окружения")
        return None
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 AI Навигатор запущен (все ключи только из переменных окружения)")
