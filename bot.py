...
def generate_text(topic: str) -> str:
    if AGNES_API_KEY:
        try:
            ...
            prompt = f"Напиши развернутый пост (около 200 слов) на тему: {topic} (искусственный интеллект, нейросети). Используй факты, примеры, выводы. Добавь эмодзи и абзацы."
            data = {
                "model": "agnes-v1",
...
def generate_template_text(topic: str) -> str:
    intro_phrases = [
        f"🧠 **{topic}** — одна из ключевых тем современного мира ИИ.\n",
        f"🤖 **{topic}** — это не просто тренд, это реальность.\n",
        f"🚀 **{topic}** — будущее, которое уже наступило.\n"
    ]
    body_phrases = [
        "📊 Исследования показывают, что внедрение AI повышает эффективность на 30–40%.",
        "💡 Нейросети уже сегодня помогают автоматизировать рутинные задачи.",
        "🔍 Специалисты по данным становятся самыми востребованными.",
        "⚡️ Однако важно помнить о вызовах: этика, безопасность и обучение.",
        "🌍 Перспективы — от персонализированной медицины до умных городов."
    ]
    conclusion_phrases = [
        "🚀 Следите за развитием технологий, чтобы не отставать!",
        "✨ Искусственный интеллект открывает новые горизонты.",
...
def generate_image(topic):
    ...
    # Перемешиваем, чтобы разнообразить порядок
    random.shuffle(generators)

    for name, func in generators:
        img = func(topic)
        if img:
            # Проверяем, не использовалась ли картинка ранее
    ...
    # Если генераторы дали дубли, пробуем Pexels
    if PEXELS_API_KEY:
    ...
    # Баннер — всегда уникален, так как содержит текст поста
    img = create_banner(topic[:20])
    # Хеш баннера будет разным для каждой темы, но если тема повторяется – можем не проверять
    if not is_image_used(img):
        mark_image_as_used(img)
        logger.info("✅ Использован баннер (уникальный)")
        return img, "баннер"
    else:
        # Если вдруг баннер тоже дублируется (маловероятно), создаём с небольшим изменением
