...
Ключи читаются из переменных окружения (или из secrets.txt как fallback).
"""
...
def _read_secret(name):
    """Сначала переменная окружения, потом файл secrets.txt рядом с ботом."""
    val = os.getenv(name, "").strip()
    if val:
        return val
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "secrets.txt")
...
VK_TOKEN        = _read_secret("AI_VK_TOKEN")
HUGGINGFACE_KEY = _read_secret("HUGGINGFACE_API_KEY")
PEXELS_KEY      = _read_secret("PEXELS_API_KEY")
...
def main():
    ...
    logger.info(f"🔑 VK: {'есть' if VK_TOKEN else 'НЕТ'} ({VK_TOKEN[:12]}...)" if VK_TOKEN else "🔑 VK: НЕТ")
...
def _diag():
    """Показывает, откуда бот пытается взять секреты."""
    env_vk = os.getenv("AI_VK_TOKEN", "")
    secrets_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "secrets.txt")
    logger.error("❌ Не найден AI_VK_TOKEN")
    logger.error(f"   Переменная окружения AI_VK_TOKEN: {'ЗАДАНА' if env_vk else 'ПУСТА'}")
    logger.error(f"   Файл secrets.txt: {'есть' if os.path.exists(secrets_path) else 'НЕТ'} → {secrets_path}")
    logger.error(f"   Рабочая папка: {os.getcwd()}")
    vk_envs = [k for k in os.environ if "VK" in k.upper() or "TOKEN" in k.upper()]
    logger.error(f"   Переменные с VK/TOKEN в окружении: {vk_envs}")
...
if __name__ == "__main__":
    ...
    # Тестовый режим: python bot.py test — публикует один пост и выходит
