...
Диагностика через Telegram: /diag, /groq
"""
...
def diag_check_groq():
    ...
    # Пробуем ту же цепочку моделей, что и основной код
    for model in GROQ_MODELS:
        try:
            ...
            if r.status_code == 200:
                ...
                return f"❌ Groq: 401 — ключ неверный или отозван"
            elif r.status_code == 429:
                return f"⚠️ Groq: 429 — превышен лимит запросов"
            elif r.status_code == 404:
                continue  # пробуем следующую модель
            else:
                return f"❌ Groq: HTTP {r.status_code} — {r.text[:150]}"
        except requests.exceptions.ConnectionError as e:
            return f"❌ Groq: нет соединения с api.groq.com ({str(e)[:100]})"
        except Exception as e:
            return f"❌ Groq: {type(e).__name__}: {str(e)[:150]}"
    return "❌ Groq: ни одна модель из списка недоступна"
...
def diag_check_vk():
    ...
    try:
        r = requests.get("https://api.vk.com/method/users.get",
                         params={"access_token": VK_TOKEN, "v": "5.199"}, timeout=15)
        d = r.json()
        if "response" in d:
            return f"✅ VK: OK (id={d['response'][0].get('id')})"
        err = d.get("error", {})
        return f"❌ VK: {err.get('error_code')} — {err.get('error_msg')}"
    except Exception as e:
        return f"❌ VK: {type(e).__name__}: {str(e)[:100]}"
...
def diag_full():
    lines = [
        ...
        f"  VK_TOKEN: {'✅ есть' if VK_TOKEN else '❌ НЕТ'}",
            ...
            for u in upd.get("result", []):
                ...
                if cmd in ("/start", "/help"):
                    tg_send(
                        ...
                        f"/diag — диагностика ключей и сервисов\n"
                        f"/groq — проверка Groq отдельно\n"
                        f"/status — состояние\n"
                        f"/types — счётчики\n\n"
                    ...
                    def _groq_report():
                        ...
                        if status.startswith("✅"):
                            ...
                            if body:
                                ...
                                extra = "\n\n⚠️ Одиночный запрос не прошёл, но диагностика ок"
...
def pexels_query_from_topic(topic_en):
    ...
    if not useful: useful = ["technology", "artificial", "intelligence"]
    ...
    for model in GROQ_MODELS:
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(f"Groq 401 — ключ неверный, дальше нет смысла")
                return None
            elif r.status_code == 429:
                logger.warning(f"Groq 429 — лимит, пробую следующую модель")
                last_err = "429"
                continue
            else:
...
FALLBACK_BY_TYPE = {
    "tip": [
        ...
        "Если нейросеть выдаёт воду — добавь в промпт фразу: «без вступлений, сразу по делу, "
        "5 пунктов». Работает почти с любой моделью. Точность ответа резко растёт.\n\n"
        "Как тебе такой приём? 👇",
    ],
    "tool": [
        ...
        "Плюс: можно уточнять вопрос в диалоге, как с обычным чатом. "
        "Бесплатной версии хватает для большинства задач.\n\n"
    ...
    "prompt": [
        ...
        "Подставь свою тему в начало — работает и для учёбы, и для презентаций.\n\n"
        "Какой промпт нужен тебе? Напиши в комментариях 👇",
    ],
    "myth": [
        ...
        "Программирование помогает в узких задачах, но 90% пользы доступно без кода.\n\n"
        "А как думал ты раньше? 👇",
    ],
    "question": [
        "Интересно ваше мнение: каким AI-инструментом вы пользуетесь каждый день "
        "и почему именно им? Не разово попробовать, а реально вернуться в работу. "
        "Поделитесь в комментариях — соберём народный рейтинг 👇",
    ],
    "quiz": [
...
def publish(text, image_data):
    ...
    if image_data:
        try:
            ...
            bio.name = "post.jpg"  # vk_api требует имя файла
            ph = safe_vk(upload.photo_wall, photos=bio, group_id=abs(GROUP_ID))
            if ph:
