...
def tg_init():
    ...
    logger.warning(f"📨 Telegram: chat_id не найден. Напиши /start боту @{bot_username}")
    return None
...
# ================== ОБРАБОТКА КОМАНД ==================
def tg_command_loop():
    """Слушает команды от пользователя в фоне."""
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        logger.info("📨 Командный цикл Telegram не запущен (нет токена/chat_id)")
        return

    logger.info("📨 Слушаю команды в Telegram: /test, /status, /help")
    offset = 0

    # Пропускаем уже накопленные апдейты
    ...
    while True:
        try:
            ...
            for u in upd.get("result", []):
                ...
                if not text:
                    continue
                # Команды принимаем только от нашего chat_id
                if chat_id != str(TELEGRAM_CHAT_ID):
                    tg_send("⛔ Нет доступа.", chat_id=chat_id)
                    continue

                cmd = text.split()[0].lower().split("@")[0]
                logger.info(f"📨 Команда из Telegram: {cmd}")

                if cmd in ("/start", "/help"):
                    tg_send(
                        f"🤖 <b>{BOT_NAME}</b>\n\n"
                        f"Команды:\n"
                        f"/test — сделать тестовый пост прямо сейчас\n"
                    ...
                    with STATE_LOCK:
                        s = dict(STATE)
                    tg_send(
                        f"📊 <b>[{BOT_NAME}] статус</b>\n\n"
                        f"Группа: {GROUP_ID}\n"
                        f"Расписание: {', '.join(POST_TIMES)}\n"
                        f"Тем доступно: {s['topics_count']}\n"
                        f"Последний пост: {s['last_post_time'] or '—'}\n"
                        f"Тема: {s['last_post_topic'] or '—'}\n"
                        f"Результат: {'✅ ок' if s['last_post_ok'] else ('❌ ошибка' if s['last_post_ok'] is False else '—')}"
                    )
                elif cmd == "/test":
                    tg_send("🧪 <b>Тестовый пост запущен</b>\nГотовлю текст и картинку...")
                    # Запускаем в отдельном потоке, чтобы не блокировать цикл
                    threading.Thread(target=post_now, daemon=True).start()
                else:
                    tg_send(f"❓ Не знаю команду <code>{cmd}</code>. Напиши /help")
        except Exception as e:
...
def main():
    ...
    # Telegram: определить chat_id и запустить прослушку команд
    tg_init()
    if TELEGRAM_CHAT_ID:
        tg_send(
