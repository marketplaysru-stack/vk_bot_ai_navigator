...
Генерация текста: Pollinations → HF → умный шаблон с переводом.
...
def tg_init():
    ...
    # Снимаем вебхук, но НЕ сбрасываем апдейты — они нужны для поиска chat_id
    ...
    logger.warning(f"📨 Telegram: chat_id неизвестен — жду сообщения от тебя. Напиши /start боту @{bot_username}")
    return None
        ...
        try:
            ...
            for u in upd.get("result", []):
                ...
                if not TELEGRAM_CHAT_ID and chat_id:
                    ...
                    logger.info(f"📨 chat_id определён из сообщения: {TELEGRAM_CHAT_ID}")
...
def translate_to_ru(text):
    """Перевод en→ru через MyMemory. Бесплатно, без ключа."""
    if not text:
        return text
    # Если уже русский — не переводим
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if translated and len(translated) > 5:
                logger.info(f"🌐 Переведено: {translated[:80]}...")
                return translated
    except Exception as e:
...
def gen_text_pollinations(topic, summary=""):
    """Pollinations text. Если вернул ошибку бюджета — считаем неудачей."""
    prompt = (
    ...
    try:
        ...
        if r.status_code == 200 and r.text:
            ...
            if len(text) > 80 and cyr > 30:
                return text
            logger.warning("Pollinations текст не на русском или короткий")
    except Exception as e:
        logger.warning(f"Pollinations текст: {e}")
    return None

def gen_text_hf(topic, summary=""):
    """Резерв: Hugging Face (если DNS починился)."""
...
def smart_fallback(topic, summary=""):
    """Осмысленный текст, даже без LLM: опирается на тему и summary."""
...
def format_post(topic, body):
    """Добавляет эмодзи-заголовок и хештеги."""
    emojis = ["🧠", "🤖", "💡", "⚡", "🚀", "🧩", "🌐", "📌", "🔍"]
...
def generate_text(topic, summary=""):
    ...
    logger.warning("⚠️ LLM недоступен — умный шаблон с переводом")
    return format_post(topic, smart_fallback(topic, summary))
...
def get_topic():
    """Возвращает (topic_ru, summary, source_url). Заголовок уже переведён на русский."""
    candidates = []
...
def gen_img_poll(prompt):
    try:
        ...
        if r.status_code == 200 and r.content:
            # Проверяем, что это картинка, а не текст ошибки
            if r.headers.get("content-type", "").startswith("image") or len(r.content) > 10000:
                return r.content
    except Exception as e:
