...
# -*- coding: utf-8 -*-
"""
Простой бот с генерацией картинок, публикацией в группу и анонсом на личную страницу
"""
...
# ========== ГЕНЕРАЦИЯ ЧЕРЕЗ AGNES (УЛУЧШЕННЫЙ ПРОМПТ) ==========
def generate_image_agnes(prompt: str) -> bytes:
    if not AGNES_API_KEY:
        raise ValueError("AGNES_API_KEY не задан")
    # Формируем промпт на английском для бизнес/технологической тематики
    enhanced_prompt = (
        f"Professional business illustration about {prompt}. "
        "Include modern technology elements: AI icons, neural network diagrams, data charts, "
        "futuristic digital interface, glowing lines, abstract circuit boards, "
        "innovation concept, high detail, 4K, vibrant colors, blue and purple palette, "
        "no people, no faces, no nature, no trees, no landscapes."
    )
    headers = {
    ...
    logger.info(f"Отправка запроса в Agnes: {enhanced_prompt[:100]}...")
...
# ========== ЗАГРУЗКА ФОТО В VK (ДЛЯ ГРУППЫ) ==========
def upload_photo_to_vk(image_bytes, group_id, token):
    if not token:
        raise ValueError("Нет VK токена")
    group_id_abs = abs(group_id) if group_id < 0 else group_id
    # 1) Получаем URL для загрузки
    upload_url_api = "https://api.vk.com/method/photos.getWallUploadServer"
    params = {"access_token": token, "group_id": group_id_abs, "v": "5.131"}
...
# ========== ПУБЛИКАЦИЯ В ГРУППУ ==========
def publish_to_group(text, image_bytes=None):
    ...
    try:
        ...
        return f"✅ Пост в группе опубликован (id: {post_id})"
    except Exception as e:
        logger.error(f"Ошибка публикации в группу: {e}")
        return f"❌ Ошибка: {e}"

# ========== ПУБЛИКАЦИЯ НА ЛИЧНУЮ СТРАНИЦУ (АНОНС) ==========
def publish_to_user(text, image_bytes=None, group_link=None):
...
def handle_command(chat_id, text):
    if text in ("/start", "/help"):
        send_message(chat_id,
            ...
            "📌 Команды:\n"
            "/post <тема> — сгенерировать и опубликовать пост (группа + анонс)\n"
    ...
    if text.startswith("/post"):
        ...
        user_result = publish_to_user(announce_text, image_bytes, group_link)
        send_message(chat_id, f"👤 Личная страница:\n{user_result}")
