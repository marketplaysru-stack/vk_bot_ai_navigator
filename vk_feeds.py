...
MAX_POSTS_PER_DAY = 4
PROCESSED_IDS_FILE = "/data/processed_vk_posts_ai.txt"
MAX_POST_AGE_DAYS = 3
MIN_TEXT_LENGTH = 50
...
def rewrite_with_agnes(original_text, niche=""):
    ...
    prompt = (
        "Ты — профессиональный копирайтер. Перепиши следующий текст в уникальный, интересный пост для блога об искусственном интеллекте. "
...
def fetch_and_generate_topics_from_vk(schedule, limit=3):
    ...
    if not token:
        logger.error("❌ VK_TOKEN_AI не задан")
        return []
    ...
    logger.info("🔍 Проверка новых постов в AI-группах ВК...")
    ...
    logger.info(f"✅ Найдено {len(new_topics)} новых тем из AI-групп ВК (осталось лимитов на сегодня: {remaining})")
    return new_topics